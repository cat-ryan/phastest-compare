...
extract_genome_length <- function(file_path, genus, species) {
  lines <- readLines(file_path) #import file 
  #RegEx to extract the genome length
  match <- regmatches(lines[32], regexpr("\\d+(?=, gc%)", lines[32], perl = TRUE))
  number <- as.numeric(match) #convert to numeric
...
#Create a data frame of genome lengths with corresponding genus and species name
genome_data <- do.call(rbind, lapply(file_paths, function(x) {
  result <- extract_genome_length(x$file, x$genus, x$species)
    return(data.frame(
...
#Function create a data frame from the phage region table in summary.txt
read_table_from_file <- function(file_path, genus, species) {
  lines <- readLines(file_path)   #read in the file
  ...
  colnames(table_data) <- unlist(header)  #add the column names to the data
  table_data$genus <- genus #add the genus
  table_data$species <- species #add the species
  return(table_data) #return the data frame
}

#Combine the data frames
combined_data <- do.call(rbind, lapply(file_paths, function(y) {
  result2 <-  read_table_from_file(y$file, y$genus, y$species)
}))
...
##########Get the % Genome that is Phage Derived##########
